...
'''
This is a Caffe implementation of Zhe Cao, et al (https://arxiv.org/pdf/1611.08050.pdf) 's
implementation of image-to-joint detector.
'''


#load images from this action class into list of videos, which is then a list of frames
def load_images(data_dir, action_class):
	__, action_dict = save_actions(data_dir)
	action_inds = action_dict[action_class]
	actiondir = data_dir + "/" + action

	#walks through each mat
	videos = []
	for folder in os.listdir(actiondir): 
		if folder.startswith('.'): #skips weird .AppleDouble -- what does this mean? 
			continue 
		video_dir = sio.loadmat(actiondir + '/' + folder)
		frames = [] #frame matrices for this video
		for frame in os.listdir(video_dir):
			if frame.startswith('.'): #skips weird .AppleDouble -- what does this mean? 
				continue 
			frame_dir = video_dir + '/' + frame
			frames.append(cv.imread(frame_dir))
		videos.append(frames)

	return videos[action_inds]

def recognize_joint(oriImg):
...
if __name__ == '__main__':
	...
	action_class = sys.argv[1]
	videos = load_images(data_dir, action_class)
